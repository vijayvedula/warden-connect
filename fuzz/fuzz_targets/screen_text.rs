...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(canonical) = canon::canonicalise(SurfaceKind::McpTools, id, &surface, &Limits::default())
    else {
        return;
    };
    ...
    for mode in [ScreenMode::Observe, ScreenMode::Flag, ScreenMode::Enforce] {
        for tier in [Tier::ONE, Tier::THREE] {
            let report = screen::screen(
                &canonical,
                tier,
                &ScreenCtx { rules: &rules, acceptances: &acceptances, names: &names,
                            entity: id, mode },
            );
            ...
            assert_eq!(report.ran.len() + report.skipped.len(), screen::Detector::ALL.len());
