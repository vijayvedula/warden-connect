...
| `--json` on `connect contracts` | accepted and **silently ignored** for the list. The single-contract view (`--cid X --json`) honoured it; the list printed its table regardless, and `--json` is a global flag so nothing refused it. A script piping the list into a parser got a table, and a tolerant parser got nothing — which reads as an estate with no contracts. Found by a drill doing exactly that and reporting "no contract exists" |
| `register --oidc-token`, documented for "IRSA, Azure workload identity, a GCP service account or a Vault identity token" | could not work for GitHub Actions, which is the issuer most estates would reach for. Its OIDC JWKS is **RSA-only** and `add_jwks` skipped every RSA key, so no key could resolve; and no CLI flag could load a JWKS document at all — `--trust-key` takes one PEM per key and handles EC and Ed25519 only. A documented, flagged, tested path that was unreachable for its most likely issuer. Found by trying to use it |
| `lock::acquire`, reporting `WC-8003 another writer holds this` | said something that was not true. On macOS a `flock` taken immediately after a release on the same inode can transiently return `EWOULDBLOCK`, so a control plane restarted right after one was stopped would be told another writer held the log when none did. Diagnosed only by instrumenting the pid and inode — three sessions of reading the code ruled out everything reachable that way and found nothing. A minimal probe then reproduced it with nothing but `acquire`: 20,000 cycles clean in isolation, blocking within one run under parallel load. Now retried briefly before the refusal, which does not weaken single-writer because `flock` still arbitrates |
