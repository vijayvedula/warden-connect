...
| Document | What it is |
|---|---|
| [pitch.html](pitch.html) | **The elevator pitch, 58 seconds, in two acts.** For architects, technology leaders and investors. **Act I is the GitOps path in two lanes** — provider and consumer each merge in their own repository under their own approvers, and neither reviews the other's pull request; the lanes join only at the control plane, which confirms both merges with the *source host* and mints. Then **where the contract lives**: a receipt in git, the register in the plane, a copy that expires at the edge. **Act II is enforcement**, with the product boundary on the algebra — `surface` is warden-connect at issuance, `scope ∩ policy` is warden at the call |
