...
| Document | What it is |
|---|---|
| [pitch.html](pitch.html) | **The walkthrough, 2:09, eleven slides, two acts.** Each slide holds its finished frame five seconds before the cut. Opens on the real problem — all five tools *reachable*: **reachable is not approved**. **Act I is GitOps in three slides** — two lanes that never touch; the disposition (pre-granted straight through, gated waits on the callee's owner, refused stops); where the contract lives. **Act II is enforcement in three** — the algebra with the product boundary on it; **Envoy, Kong and the inline mediator side by side with their real integration details**; and the `WC-4002` refusal |
