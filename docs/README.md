...
| Document | What it is |
|---|---|
| [pitch.html](pitch.html) | **The walkthrough, 2:23, twelve slides, two acts.** Each slide holds five seconds before the cut. The problem — all five tools *reachable*: **reachable is not approved**. **Act I is GitOps** — two lanes that never touch (GitHub · GitLab · Azure Repos · Bitbucket); the disposition (pre-granted, gated, refused); where the contract lives. **Act II is enforcement** — the algebra with the product boundary on it; Envoy, Kong and the inline mediator with their integration details; the same `WC-4002` refused in all three. Closes on **the logical architecture, every capability in one frame** |
