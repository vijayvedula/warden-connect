...
| Document | What it is |
|---|---|
| [pitch.html](pitch.html) | **The elevator pitch, 58 seconds, in two acts.** Opens on the real problem, stated technically: a token names a service, a server exposes five tools, and all five are *reachable* — **reachable is not approved**. **Act I is the GitOps path in two lanes** — provider and consumer each merge in their own repository, neither reviews the other's pull request, and the lanes join only at the control plane, which confirms both merges with the *source host*. Then where the contract lives. **Act II is enforcement**, with the product boundary on the algebra: `surface` is warden-connect at issuance, `scope ∩ policy` is warden at the call |
