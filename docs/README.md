...
| Document | What it is |
|---|---|
| [pitch.html](pitch.html) | **The walkthrough, 1:56, ten slides, two acts.** Each slide holds its finished frame five seconds before the cut. Opens on the real problem — a token names a service, a server exposes five tools, all five *reachable*: **reachable is not approved**. **Act I is GitOps in three slides** — two lanes that never touch; the disposition (pre-granted straight through, gated waits on the callee's owner, refused stops); and where the contract then lives. **Act II is enforcement**, with the product boundary drawn on the algebra |
